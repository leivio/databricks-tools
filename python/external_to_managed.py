...
class ExternalToManaged:
    ...
    def __init__(self, catalog, schema=None, table=None, like=None, owner="AADS_A_BEES_UC_FRONTLINE_FORCE_ENGINEER"):
...
ExternalToManaged(catalog="frt_am_uat", schema='gold_force', like="kpi_").run()

# COMMAND ----------

ExternalToManaged(catalog="frt_am_uat", schema='gold_force', table="external__sharing_visit").pretty()

# COMMAND ----------

spark.read.table(f"frt_am_uat.information_schema.column_tags").filter(f"catalog_name = 'frt_am_uat' and schema_name = 'gold_force' and table_name = 'feature__bees_one_export_visit'").display()

# COMMAND ----------

ExternalToManaged(catalog="frt_am_uat", schema='gold_force', like="kpi_").information_tables.display()

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from frt_am_uat.information_schema.tables where table_type = 'EXTERNAL' and table_schema = 'gold_force'